...
- 浮动找浮动，不浮动找不浮动
- 浮动的元素以自己在没有脱离标准流之前的位置
- 浮动会改变元素的显示方式：inline-block
...
## 绝对定位（absolute）
- 绝对定位通过top、left、right、bottom来定义位置

绝对定位需要注意以下几点：
1. 如果定位的盒子没有父元素，那么这个盒子是相对于body的
2. 如果定位的盒子有父元素，但父元素没有定位，那么这个盒子是相对于body的
3. 如果定位的盒子有父元素，并且父元素有定位，那么这个盒子是相对于父元素的
4. 绝对定位的盒子不占页面位置（脱离标准流）
5. 如果没有设置top、left那么这个盒子就是设置了position属性，位置也不会改变
...
# 图片的居中：vertical-align

- 如果文字和图片显示在一行中，那么默认情况下，图片的底部与文字的基线对齐。
- 如果文字和图片在一行，给图片设置margin-top，文字也会跟着图片一起下移。
- 如果要让文字的中线和图片的中线对齐，使用vertical-align:middle。
